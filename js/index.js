...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (heroSection) {
    const obs = new IntersectionObserver(entries => {
      if (entries[0].isIntersecting && !countersDone) {
        ...
        animateCounter(document.getElementById('counterEff'),  92.4, '%');
  ...
  if (typeof gsap !== 'undefined' && typeof ScrollTrigger !== 'undefined') {
    ...
    gsap.from('.hero__dashboard-card', {
      y: 40, opacity: 0, duration: 1, delay: 0.5,
      ease: 'power3.out',
    });

    // Stats banner numbers pop
    ...
    // Map dots pulse
    gsap.to('.map-dot-group circle:nth-child(odd)', {
      scale: 1.3, duration: 1.2, repeat: -1, yoyo: true,
      ease: 'sine.inOut', stagger: 0.2,
      transformOrigin: 'center center',
    });
    gsap.to('.map-dot-group-warn circle:nth-child(odd)', {
      scale: 1.5, duration: 0.8, repeat: -1, yoyo: true,
      ease: 'sine.inOut',
      transformOrigin: 'center center',
    });
    ...
    if (chartLine) {
      ...
      gsap.to(chartLine, {
        strokeDashoffset: 0, duration: 2, delay: 0.8,
        ease: 'power2.out',
      });
    }
  }
